...
class AirCargoProblem(Problem):
    def __init__(self, cargos, planes, airports, initial: FluentState, goal: list):
        ...
        fluSta = decode_state(self.initial_state_TF,self.state_map)
        self.neg_list = fluSta.neg
        
        blank_list = []
        self.opposite_goal_state = FluentState(blank_list,goal)
        self.opposite_goal_state_map = blank_list + goal
        self.opposite_goal_state_TF = encode_state(self.opposite_goal_state,self.opposite_goal_state_map)
        oppGoal_fluSta = decode_state(self.opposite_goal_state_TF,self.opposite_goal_state_map)
        self.goal_list = oppGoal_fluSta.neg
    ...
    def get_actions(self):
            ...
            for c in self.cargos:
                for a in self.airports:
                        for p in self.planes:
                            precond_pos = [expr("At({}, {})".format(c, a)),#Cargo #Plane
                                           expr("At({}, {})".format(p, a))
                                           ]
                            precond_neg = [
                                           ]
        ...
        def unload_actions():
            ...
            #unload_precond_pos = In(c, p) ∧ At(p, a) ∧ Cargo(c) ∧ Plane(p) ∧ Airport(a)
            #unload_precond_neg =
        ...
        def fly_actions():
            ...
            for fr in self.airports:
                for to in self.airports:
                    if fr != to:
                        for p in self.planes:
                            precond_pos = [expr("At({}, {})".format(p, fr)), #Plane #From
                                           ]
                            precond_neg = []
...
#        for goal in self.actions_list:
#            if goal in self.state_map:
#                
#                
#                for ch in self.actions_list:
#                    if (ch == "T" and ch != "F") or (ch == "F" and ch != "T"):
#                        if action not in my_goal_list:
#                            my_goal_list.insert(action)

        
        #union = [x for x in self.initial.neg_list and AirCargoProblem.goal]
        #count = len(self.negList)
        #count = len(self.actions_list)

        #self.actions_list = actions_list_swap 
        count = 0
        
        intersection = [x for x in self.neg_list and self.goal_list]
...
def air_cargo_p2() -> AirCargoProblem:
    # TODX implement Problem 2 definition
...
def air_cargo_p3() -> AirCargoProblem:
    # TODX implement Problem 3 definition
